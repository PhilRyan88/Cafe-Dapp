...
async function main() {
  ...
  console.log("Before buying cafe");
  await cosoleBalances(addresses);

  const amount = { value: hre.ethers.utils.parseEther("1") };
  await contract.connect(from1).buy("from1", "Very nice coffee", amount);
  await contract.connect(from2).buy("from2", "Great coffee", amount);
  await contract.connect(from3).buy("from3", "Good coffee", amount);
